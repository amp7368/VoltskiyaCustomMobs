...
public class MobFireballThrow<Config extends FireballThrowConfig> extends MobToTick<Config> {
    ...
    private final SoundManager soundManager = new SoundManager()
            .registerSound(new SoundActionImpl(SOUND_THROW, Sound.ENTITY_BLAZE_SHOOT, SoundCategory.HOSTILE, .2f, 1.2f))
            .registerSound(new SoundActionImpl(SOUND_WARNING, Sound.ENTITY_HUSK_HURT, SoundCategory.HOSTILE, 0.7f, 1.4f));
    private final RepeatingActionManager actionManager = MobTickPlugin.get().createRepeatingAction()
            .registerInit(this::initThrow)
            .registerAction(new OneOffAction(CHARGE_UP, this::doChargeUp))
            .registerAction(new OneOffAction(DO_THROW, this::doThrow))
            .registerFinally(this::quitThrow);
    ...
    private boolean shouldDoAbility() {
        ...
        double distanceToTarget = DistanceUtils.distance(player.getLocation(), getLocation());
        if (!getBukkitMob().hasLineOfSight(player)) return false;
    ...
    private void doThrow() {
        ...
        @NotNull Vector direction = target.getEyeLocation().clone().subtract(location).toVector().normalize();
    ...
    protected void shootFireball(Location location, @NotNull Vector direction) {
        @NotNull Entity entity = location.getWorld().spawnEntity(location, EntityType.SMALL_FIREBALL, CreatureSpawnEvent.SpawnReason.CUSTOM);
