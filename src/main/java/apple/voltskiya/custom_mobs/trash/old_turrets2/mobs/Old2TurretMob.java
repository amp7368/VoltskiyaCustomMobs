...
import apple.voltskiya.custom_mobs.trash.old_turrets2.Old2TurretList;
...
public abstract class Old2TurretMob implements Runnable {
    ...
    protected Old2TurretMob(String typeId) {
        this.typeId = typeId;
        for (int i = 0; i < ARROW_SLOT; i++)
            arrows.add(new DBItemStack(null, 0, new NBTTagCompound().asString()));
    }

    public Old2TurretMob(String typeId, Old2TurretMobSaveable saveable) {
        this.typeId = typeId;
    ...
    protected DBItemStack removeArrow() {
        for (DBItemStack arrow : arrows) {
            ...
            if (arrow.type != null && arrow.type != Material.AIR && count > 0) {
                ...
                updateGui();
                Old2TurretList.registerOrUpdate(this);
                return arrow;
            }
        }
        return null;
    ...
    public abstract Old2TurretMobSaveable toSaveable();
