...
public class OldReviverManagerTicker extends ConfigManager implements RegisteredEntityEater {
    public int REVIVE_RITUAL_TIME;
    public double REVIVE_DISTANCE;
    public double REVIVE_CHANCE;
    private static OldReviverManagerTicker instance;
    private final Map<Closeness, OldReviverIndividualTicker> closenessToReviveres = new HashMap<>() {{
        for (Closeness closeness : Closeness.values())
            put(closeness, new OldReviverIndividualTicker(closeness));
        get(Closeness.HIGH_CLOSE).setIsReviving();
    }};


    public OldReviverManagerTicker() throws IOException {
        instance = this;
    ...
    public static OldReviverManagerTicker get() {
        return instance;
    }
...
//        closenessToReviveres.get(closeness).giveReviver(new Reviver(reviver,));
    ...
    public boolean amIGivingReviver(MobReviver reviver, Closeness currentCloseness) {
...
//        }
    ...
    private Closeness determineConcern(@Nullable Entity reviver) {
        ...
        @Nullable Player player = UpdatedPlayerList.getClosestPlayerPlayer(reviverLocation);
