...
public class LeapUpwards {
    public static void eatSpawnEvent(CreatureSpawnEvent event, LeapType leapType) {
        ...
        if (creature instanceof EntityInsentient) {
            ((EntityInsentient) creature).goalSelector.a(1, new PathfinderGoalLeapUpwards((EntityInsentient) creature, leapType, postConfig));
        }
    }


    private static void preLeap(EntityInsentient entity, Runnable leap) {
        Location location = entity.getBukkitEntity().getLocation();
        LeapSounds.CHARGE_UP_GROWL.accept(location);
        Bukkit.getScheduler().scheduleSyncDelayedTask(VoltskiyaPlugin.get(), leap, 10);
    }
