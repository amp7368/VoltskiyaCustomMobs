...
public class LeapPounce {
    ...
    public static void eatEntity(EntityLiving creature, LeapPreConfig config) {
        if (creature instanceof Mob insentient) {
            ...
            final GoalLeap pounce = new GoalLeap((Mob) creature, config, postConfig);
            pounce.setMoveType(EnumSet.of(DecodeMoveType.TARGET.encode(), DecodeMoveType.JUMP.encode()));
            DecodeEntity.getGoalSelector((Mob) creature).a(0, pounce);
        }
    }
    ...
    private static void preLeap(Mob entity, LeapDo leapDo, int timeLeft, int index) {
